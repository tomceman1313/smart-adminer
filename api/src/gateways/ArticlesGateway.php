...
class ArticlesGateway
{
    ...
    public function create(array $data, int $userId): bool
    {
        ...
        foreach ($innerImages as $image) {
            $this->createImage($image["name"], $image["file"], $last_id);
        }

        $images = $data["images"];

        foreach ($images as $image) {
            $this->createImage(uniqid(), $image, $last_id);
        }
    ...
    public function get(string $id): array
    {
        ...
        $sql = "SELECT id, name FROM article_images WHERE article_id = :id";
        $stmt = $this->conn->prepare($sql);
    ...
    private function createImage($image_name, $base64, $article_id)
    {
        ...
        $sql = "INSERT INTO article_images (name, article_id) VALUES (:name, :article_id)";
        $stmt = $this->conn->prepare($sql);
